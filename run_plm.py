    ...
    def validate():
        ...
        with torch.no_grad():
            ...
            for i, (history, future, video_user_info) in enumerate(dataloader_valid):
                if i >= 50:  # ← AdaLoRA run과 동일하게 앞 50배치만 (검증 속도/기준 일치)
                    break
...
def run(args):
    ...
    if args.compile:
        ...
        print(
            "\033[33mWarning:\033[0m There seems to be some bugs in torch.compile. If batch size is too large, it will raise errors (I don't know why this happens).")
        prompt_model = torch.compile(prompt_model).to(
            args.device)  # recommend to compile model when you are using PyTorch 2.0
