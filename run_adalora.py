...
from models.low_rank_adalora import peft_model, print_trainable_parameters

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))


def _run_suffix(args):
    """Distinguish the shapley run from the original(sensitivity) run.
    MUST be identical in adapt() and test() so test loads the model adapt saved,
    and so the two runs never collide on the same output path/CSV."""
    return '_shapley' if getattr(args, 'use_shapley', False) else '_original'
...
def _resize_adalora_to_ckpt(plm, adapter_bin, adapter_name='default'):
    ...
    for k, v in sd.items():
        if 'lora_E' not in k:
            continue
        mod_key = k[:k.find('.lora_E')]  # base_model.model.transformer.h.0.attn.c_attn
        tgt = name2mod.get(mod_key)
        if tgt is None:
            continue
        r = v.shape[0]  # 이 레이어의 최종 rank (42, 48, 60, 58...)
        if r == 0:            # 완전히 pruning된 모듈: 빈 텐서로 맞추고 resize 스킵
            ...
            tgt.lora_A[adapter_name] = nn.Parameter(torch.zeros(0, in_f, device=dev, dtype=dt))
            tgt.lora_B[adapter_name] = nn.Parameter(torch.zeros(out_f, 0, device=dev, dtype=dt))
            tgt.lora_E[adapter_name] = nn.Parameter(torch.zeros(0, 1, device=dev, dtype=dt))
            continue
        in_f = tgt.lora_A[adapter_name].shape[1]
        out_f = tgt.lora_B[adapter_name].shape[0]
        dev, dt = tgt.lora_A[adapter_name].device, tgt.lora_A[adapter_name].dtype
        tgt.lora_A[adapter_name] = nn.Parameter(torch.zeros(r, in_f, device=dev, dtype=dt))
        tgt.lora_B[adapter_name] = nn.Parameter(torch.zeros(out_f, r, device=dev, dtype=dt))
        tgt.lora_E[adapter_name] = nn.Parameter(torch.zeros(r, 1, device=dev, dtype=dt))
...
def load_model(args, model, model_dir):
    ...
    """
    if args.rank != -1:
        _resize_adalora_to_ckpt(model.plm, os.path.join(model_dir, 'adapter_model.bin'))  # ← 이 줄 추가
...
def run_evaluate_result(args, result_path, results_dir, file_prefix):
    """테스트가 끝난 뒤 evaluate_result.py 를 자동 실행한다.

    ┌───────────────────────────────────────────────────────────────────┐
    │ ▼▼ 중요 ▼▼                                                          │
    │ evaluate_result.py 의 실제 인자 규약에 맞게 아래 eval_cmd 를 수정해라. │
    │ 지금은 '결과 CSV 경로를 첫 번째 위치인자로 받는다' 고 가정한다.       │
    │ 만약 --result-path 같은 플래그를 쓰면 그 형태로 바꾸면 된다.         │
    └───────────────────────────────────────────────────────────────────┘
    """
    if getattr(args, 'skip_eval', False):
        print('[eval] --skip-eval 지정됨, evaluate_result.py 실행 생략')
        return

    eval_script = args.eval_script if args.eval_script else os.path.join(SCRIPT_DIR, 'evaluate_result.py')
    if not os.path.exists(eval_script):
        print(f'\033[33m[eval] Warning:\033[0m {eval_script} 없음, evaluate_result.py 실행 생략')
        return

    # ── evaluate_result.py 호출 커맨드 (네 스크립트 인자에 맞게 수정) ──
    eval_cmd = [sys.executable, eval_script, result_path]
    # 예) 플래그형이라면:
    # eval_cmd = [sys.executable, eval_script, '--result-path', result_path, '--results-dir', results_dir]
    # ────────────────────────────────────────────────────────────────

    print('[eval] 실행:', ' '.join(eval_cmd))
    try:
        subprocess.run(eval_cmd, cwd=SCRIPT_DIR, check=False)
    except Exception as e:  # 평가 실패가 학습/테스트 결과를 날리지 않도록 방어
        print(f'\033[33m[eval] Warning:\033[0m evaluate_result.py 실행 중 오류: {e}')
...
def adapt(args, pipeline, dataloader_train, dataloader_valid, models_dir, grad_accum_steps):
    ...
    global_step = 0
    opt_step = 0  # AdaLoRA: counts actual optimizer update steps (must match total_step)
    ...
    def validate():
        ...
        with torch.no_grad():
            ...
            os.makedirs(validata_checkpoint_path, exist_ok=True)
            valid_loss = []
            for i, (history, future, video_user_info) in enumerate(dataloader_valid):
                if i >= 50:  # ← 앞 50배치만
                    break
    ...
    for epoch in range(args.epochs):
        ...
        for step, (history, future, video_user_info) in enumerate(dataloader_train):
            ...
            if ((step + 1) % grad_accum_steps == 0) or (step + 1 == len(dataloader_train)):
                optimizer.step()
                # --- AdaLoRA rank reallocation ---
                # MUST be called after optimizer.step() and BEFORE zero_grad(),
                # because the importance scores are computed from parameter grads.
                # If your peft version raises AttributeError here, replace
                # `pipeline.plm.base_model` with `pipeline.plm`.
            ...
            if global_step % report_loss_per_steps == 0:
                print("Epoch {}, global_step {}, average loss: {}".format(epoch, global_step, (
                            tot_loss - log_loss) / report_loss_per_steps), flush=True)
                log_loss = tot_loss
            ...
            if args.save_checkpoint_per_step is not None and global_step % args.save_checkpoint_per_step == 0:
                save_checkpoint_path = os.path.join(checkpoint_path,
                                                    str(global_step // args.save_checkpoint_per_step))  # save checkpoint
...
def test(args, pipeline, dataloader_test, models_dir, results_dir):
    file_prefix = f'his_{args.his_window}_fut_{args.fut_window}_ss_{args.sample_step}_epochs_{args.epochs}_bs_{args.bs * args.grad_accum_steps}_' \
                  f'lr_{args.lr}_seed_{args.seed}_rank_{args.rank}_scheduled_sampling_{args.scheduled_sampling}'
    file_prefix += _run_suffix(args)  # ← adapt() 와 동일한 접미사 (모델 로드/CSV 충돌 방지)
    best_model_path = os.path.join(models_dir, file_prefix, 'best_model')
    ...
    if os.path.exists(model_path):
        ...
        if args.rank != -1:
            ...
            df = extract_final_ranks(pipeline.plm)  # PEFT로 감싼 GPT-2
            print(df)
            plot_rank_heatmap(
                df,
                out=os.path.join(results_dir, file_prefix + '_adalora_ranks.png'),
                vmax=None,  # 아래 설명 참고
            )
        # ─────────────────────────────
    else:
    ...
    from latency_benchmark import run_test_with_benchmark

    run_test_with_benchmark(args, pipeline, dataloader_test, notebook,
                            result_path, results_dir, file_prefix,
                            warmup=10, tag=f'adalora_rank{args.rank}')

    # ── 테스트가 끝나면 evaluate_result.py 자동 실행 ──
    run_evaluate_result(args, result_path, results_dir, file_prefix)
    # ────────────────────────────────────────────────
...
def run(args):
    ...
    # NOTE: output dirs are tagged with '_adalora' so this run never collides
    # with the original '_low_rank' (LoRA) run, even at the same seed.
    if args.rank != -1:
        models_dir = os.path.join(cfg.plms_finetuned_dir, f'{args.plm_type}_{args.plm_size}_adalora',
                                  f'freeze_plm_{args.freeze_plm}', args.train_dataset, f'{args.dataset_frequency}Hz')
        results_dir = os.path.join(cfg.results_dir, f'{args.plm_type}_{args.plm_size}_adalora',
        ...
        results_dir = os.path.join(cfg.results_dir, f'{args.plm_type}_{args.plm_size}',
                                   f'freeze_plm_{args.freeze_plm}', args.test_dataset, f'{args.dataset_frequency}Hz')
    # exist_ok=True: 두 프로세스(original/shapley)가 공유 상위 폴더를 동시에 만들어도 안전
    os.makedirs(models_dir, exist_ok=True)
    os.makedirs(results_dir, exist_ok=True)

    # args.device_out and args.device_mid are used for model parallelism (currently only necessary for llama)
    plm, tokenizer, _ = load_plm(args.plm_type, os.path.join(cfg.plms_dir, args.plm_type, args.plm_size),
                                 plm_size=args.plm_size,
                                 device_input_side=args.device, device_output_side=args.device_out,
                                 device_middle_side=args.device_mid)
    if (
            args.plm_type == 'opt' or args.plm_type == 'gpt2') and args.plm_size != 'large':  # other plm can simply be loaded on one device
        plm = plm.to(args.device)

    # --- AdaLoRA needs total_step (number of optimizer updates) BEFORE wrapping ---
    # So we build the training dataloaders first, then wrap the plm.
    dataloader_train = dataloader_valid = None
    total_step = 1  # placeholder (used only when running test-only, no adaptation)
    if args.adapt:
        raw_dataset_train, raw_dataset_valid = create_dataset(args.train_dataset, his_window=args.his_window,
                                                              fut_window=args.fut_window, trim_head=args.trim_head,
                                                              trim_tail=args.trim_tail,
                                                              include=['train', 'valid'],
                                                              frequency=args.dataset_frequency, step=args.sample_step)
    ...
    if args.rank != -1:
        plm = peft_model(plm, args.plm_type, args.rank, total_step=total_step,
                         use_shapley=args.use_shapley, n_perm=args.shapley_perm)

    # --- (선택) gradient checkpointing: 20GB 카드에서 shapley run 헤드룸 확보용 ---
    # 주의: PEFT + frozen base 조합에서는 base 입력에 grad 가 흐르도록
    #      enable_input_require_grads() 가 필요할 수 있다(어댑터로 grad 전파).
    #      wrapper 가 이 메서드를 노출하지 않으면 경고만 내고 넘어간다.
    if args.grad_ckpt:
        enabled = False
        for obj in (plm, getattr(plm, 'base_model', None)):
            if obj is None:
                continue
            if hasattr(obj, 'gradient_checkpointing_enable'):
                try:
                    ...
                    enabled = True
                    print('[grad_ckpt] gradient checkpointing enabled on', type(obj).__name__)
                    break
                except Exception as e:
                    print(f'\033[33m[grad_ckpt] Warning:\033[0m {e}')
        if not enabled:
            print('\033[33m[grad_ckpt] Warning:\033[0m gradient_checkpointing_enable 를 찾지 못함, 건너뜀')
    ...
    if args.plm_type == 'opt' and args.plm_size == 'xxs':
        networking_head = NetworkingHead(input_dim=512, output_dim=out_dim, fut_window=args.fut_window).to(
            args.device_out)
    else:
        networking_head = NetworkingHead(input_dim=input_dim, output_dim=out_dim, fut_window=args.fut_window).to(
            args.device_out)
    ...
    pipeline = Pipeline(plm, fut_window=args.fut_window, device=args.device, embed_size=embed_size,
                        frequency=args.dataset_frequency, using_multimodal=args.using_multimodal,
    ...
    if args.compile:
        ...
        print(
            "\033[33mWarning:\033[0m torch.compile is buggy in this codebase (references an undefined variable). Do NOT use --compile.")
        raise RuntimeError('--compile is not supported in run_adalora.py')

    torch.set_float32_matmul_precision('high')

    if args.adapt:
        # ── Shapley online: allocator 에 v(S)=고정 valid 배치 loss 콜백 주입 ──
        if args.rank != -1 and args.use_shapley:
            from shapley_rank import make_loss_fn
            loss_fn = make_loss_fn(pipeline, dataloader_valid, args,
                                   n_batches=args.shapley_batches)
            pipeline.plm.base_model.rankallocator.set_loss_fn(loss_fn)
            print('[shapley] loss_fn wired to allocator (valid batches)')
        # ──────────────────────────────────────────────────────────────
        adapt(args, pipeline, dataloader_train, dataloader_valid, models_dir, args.grad_accum_steps)

    if args.test:
        # 학습 직후 자동 테스트: adapt 에서 쓴 학습 메모리를 비워 20GB 헤드룸 확보
        if args.adapt and torch.cuda.is_available():
            torch.cuda.empty_cache()

        raw_dataset_test = create_dataset(args.test_dataset, his_window=args.his_window, fut_window=args.fut_window,
                                          trim_head=args.trim_head, trim_tail=args.trim_tail, include=['test'],
                                          frequency=args.dataset_frequency, step=args.sample_step)[0]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process the input parameters to train the network (AdaLoRA variant).')
    ...
    parser.add_argument('--epochs', action="store", dest='epochs', help='(Optional) Neural network learning epochs.',
                        type=int)
    ...
    parser.add_argument('--multimodal', action="store_true", dest='using_multimodal',
                        help='using multimodal image features.')
    parser.add_argument('--save-checkpoint-per-epoch', action="store", dest='save_checkpoint_per_epoch',
                        help='save checkpoint per epoch', type=int)
    parser.add_argument('--save-checkpoint-per-step', action="store", dest='save_checkpoint_per_step',
                        help='save checkpoint per step', type=int)
    parser.add_argument('--rank', action="store", dest='rank', help='the rank of low rank matrices', type=int,
                        default=-1)
    ...
    parser.add_argument('--use-shapley', action='store_true', dest='use_shapley',
                        help='AdaLoRA importance 를 Shapley value 로 교체 (online)')
    parser.add_argument('--shapley-perm', type=int, default=3, dest='shapley_perm',
                        help='Shapley Monte-Carlo 순열 수 (작을수록 빠름)')
    parser.add_argument('--shapley-batches', type=int, default=4, dest='shapley_batches',
                        help='v(S) 계산에 쓸 고정 valid 배치 수')
    ...
    parser.add_argument('--grad-ckpt', action='store_true', dest='grad_ckpt',
                        help='(Optional) gradient checkpointing 활성화 (20GB 카드 shapley run 헤드룸 확보용)')
    parser.add_argument('--eval-script', action='store', dest='eval_script', default=None,
                        help='(Optional) 테스트 후 실행할 evaluate_result.py 경로 (기본: run_adalora.py 와 같은 폴더)')
    parser.add_argument('--skip-eval', action='store_true', dest='skip_eval',
                        help='(Optional) 테스트 후 evaluate_result.py 자동 실행 생략')
